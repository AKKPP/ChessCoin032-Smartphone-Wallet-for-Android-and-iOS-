...
class BitcoinAverage(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return dict([(r.replace("CHESS", ""), Decimal(json[r]['last']))
    ...
    def request_history(self, ccy):
        history = self.get_csv('apiv2.bitcoinaverage.com',
                               "/indices/global/history/CHESS%s?period=alltime&format=csv" % ccy)
...
class BitcoinVenezuela(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        rates = [(r, json['CHESS'][r]) for r in json['CHESS']
                 if json['CHESS'][r] is not None]  # Giving NULL for LTC
        return dict(rates)
    ...
    def request_history(self, ccy):
        return self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=CHESS")[ccy +'_CHESS']


class Bitbank(ExchangeBase):

    def get_rates(self, ccy):
        json = self.get_json('public.bitbank.cc', '/CHESS_jpy/ticker')
...
class Bitmarket(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.bitmarket.pl', '/json/CHESSPLN/ticker.json')
...
class CHESSChina(ExchangeBase):

    def get_rates(self, ccy):
        json = self.get_json('data.CHESSchina.com', '/data/ticker')
        return {'CNY': Decimal(json['ticker']['last'])}


class CHESSParalelo(ExchangeBase):

    def get_rates(self, ccy):
        json = self.get_json('CHESSparalelo.com', '/api/price')
...
class Coinbase(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return dict([(r[7:].upper(), Decimal(json[r]))
                     for r in json if r.startswith('CHESS_to_')])
...
class WEX(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json_eur = self.get_json('wex.nz', '/api/3/ticker/CHESS_eur')
        json_rub = self.get_json('wex.nz', '/api/3/ticker/CHESS_rur')
        json_usd = self.get_json('wex.nz', '/api/3/ticker/CHESS_usd')
        return {'EUR': Decimal(json_eur['CHESS_eur']['last']),
                'RUB': Decimal(json_rub['CHESS_rur']['last']),
                'USD': Decimal(json_usd['CHESS_usd']['last'])}
...
class Zaif(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('api.zaif.jp', '/api/1/last_price/CHESS_jpy')
...
class FxThread(ThreadJob):
    ...
    def format_amount(self, CHESS_balance):
        rate = self.exchange_rate()
        return '' if rate.is_nan() else "%s" % self.value_str(CHESS_balance, rate)

    def format_amount_and_units(self, CHESS_balance):
        rate = self.exchange_rate()
        return '' if rate.is_nan() else "%s %s" % (self.value_str(CHESS_balance, rate), self.ccy)

    def get_fiat_status_text(self, CHESS_balance, base_unit, decimal_point):
        rate = self.exchange_rate()

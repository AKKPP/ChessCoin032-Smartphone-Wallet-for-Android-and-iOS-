...
try:
    import hid
    from CHESShip.CHESShipComm import HIDDongleHIDAPI, DongleWait
    from CHESShip.CHESShip import CHESShip
    from CHESShip.CHESShipUtils import compress_public_key,format_transaction, get_regular_input_script, get_p2sh_input_script
    from CHESShip.bitcoinTransaction import bitcoinTransaction
    from CHESShip.CHESShipFirmwareWizard import checkFirmware, updateFirmware
    from CHESShip.CHESShipException import CHESShipException
    CHESSHIP = True
    CHESSHIP_DEBUG = is_verbose
except ImportError:
    CHESSHIP = False
...
class Ledger_Client():
    def __init__(self, hidDevice):
        self.dongleObject = CHESShip(hidDevice)
...
class LedgerPlugin(HW_PluginBase):
    libraries_available = CHESSHIP
    keystore_class = Ledger_KeyStore
    client = None
    DEVICE_IDS = [
                   (0x2581, 0x1807), # HW.1 legacy CHESShip
    ...
    def CHESShip_is_connected(self, keystore):
        try:
    ...
    def get_CHESShip_device(self, device):
        ledger = False
        ...
        return HIDDongleHIDAPI(dev, ledger, CHESSHIP_DEBUG)

    def create_client(self, device, handler):
        if handler:
            self.handler = handler

        client = self.get_CHESShip_device(device)
